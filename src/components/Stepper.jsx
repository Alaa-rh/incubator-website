    ...
    <div className={`flex justify-center items-center gap-4 ${className}`}>
      {steps.map((step, idx) => (
        <div key={idx} className="flex items-center gap-2">
          <div className={`w-15 h-15 flex items-center justify-center rounded-full text-sm font-bold border
          ...
          {idx < steps.length - 1 && <div className="w-15 h-px bg-gray-300 mx-1" />}
        </div>
      ))}
    </div>
  )
