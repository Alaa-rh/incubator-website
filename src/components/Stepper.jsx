...
const Stepper = ({ steps = [], current = 0, className = "" }) => {
  return (
    <div className={`flex justify-center items-center gap-4 m-2 ${className}`}>
      {steps.map((step, idx) => (
        <div key={idx} className="flex items-center gap-2">
          <div className={`w-20 h-20 px-4 flex items-center justify-center rounded-full text-sm font-bold border
          ...
          {idx < steps.length - 1 && <div className="w-20 h-px bg-gray-300 mx-1" />}
        </div>
      ))}
    </div>
  )
